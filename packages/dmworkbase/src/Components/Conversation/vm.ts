...
export type ConversationRenderItem =
  | ConversationRenderMessageItem
  | ConversationRenderFoldSessionItem;
...
export default class ConversationVM extends ProviderListener {
  ...
  static foldSessionPreview: Map<
    string,
    { participants: string[]; count: number }
  > = new Map(); // 会话列表折叠预览缓存
  ...
  private get supportsFolding(): boolean {
    return (
      this.channel.channelType === ChannelTypeGroup ||
  ...
  buildRenderItems(
    messages: MessageWrap[],
    allowFoldAnimation: boolean = false
    ...
    const flushPendingSession = (isActive: boolean) => {
      ...
      if (pendingSessionMessages.length >= 2) {
        ...
        const lastMessage =
          pendingSessionMessages[pendingSessionMessages.length - 1];
        const shouldAnimate =
          allowFoldAnimation &&
          isActive &&
          pendingSessionMessages.length === 2 &&
          !previousState;
        ...
        renderItems.push({
          ...
          session: {
            ...
            expandedMessages: getFoldSessionExpandedMessages({
              messages: pendingSessionMessages,
            }),
            lastMessage,
    ...
    const isStillActive =
      lastPendingMsg !== null &&
      !this.pullupHasMore &&
      nowSec - lastPendingMsg.timestamp < 120;
    ...
    if (isStillActive && lastPendingMsg) {
      ...
      this.foldSessionActiveTimer = setTimeout(() => {
        ...
        const conversation =
          WKSDK.shared().conversationManager.findConversation(this.channel);
        if (conversation) {
    ...
    for (const typingMessage of typingMessages) {
      ...
      const typingFromBot =
        typingMessage.fromUID &&
        WKSDK.shared().channelManager.getChannelInfo(
          new Channel(typingMessage.fromUID, ChannelTypePerson)
        )?.orgData?.robot === 1;
      if (
        lastItem?.type === "foldSession" &&
        lastItem.session.isActive &&
        typingFromBot
      ) {
    ...
    for (let i = 1; i < renderItems.length; i++) {
      if (
        renderItems[i - 1].type === "foldSession" &&
        renderItems[i].type === "message"
      ) {
    ...
    const lastNonTypingItem = renderItems
      .filter(
        (item) =>
          !(
            item.type === "message" &&
            item.message.contentType === MessageContentTypeConst.typing
          )
      )
      .pop();
    if (
      lastNonTypingItem &&
      lastNonTypingItem.type === "foldSession" &&
      lastNonTypingItem.session.isActive
    ) {
  ...
  private ensureBotChannelInfos() {
    ...
    for (const msg of this.messagesOfOrigin) {
      if (!msg.send && msg.fromUID && !seenUIDs.has(msg.fromUID)) {
        ...
        const ci = WKSDK.shared().channelManager.getChannelInfo(
          new Channel(msg.fromUID, ChannelTypePerson)
        );
        if (!ci) {
  ...
  findFoldSessionByMessageSeq(
    messageSeq: number
  ): FoldSessionViewModel | undefined {
    const sessionId = this.messageSeqToFoldSessionId.get(messageSeq);
    if (!sessionId) {
      return;
    }
    const renderItem = this.renderItems.find(
      (item) =>
        item.type === "foldSession" && item.session.sessionId === sessionId
    );
  ...
  toggleFoldSession(sessionId: string) {
    const session = this.renderItems.find(
      (item) =>
        item.type === "foldSession" && item.session.sessionId === sessionId
    );
  ...
  didMount(): void {
    ...
    this.messageListener = (message: Message) => {
      ...
      if (message.streamNo) {
        ...
        if (existMsg) {
          ...
          // 去重：跳过已存在的 streamSeq
          const exists = existMsg.message.streams.some(
            (s) => s.streamSeq === streamSeq
          );
          if (!exists) {
    ...
    this.channelInfoListener = (channelInfo: ChannelInfo) => {
      ...
      const hasBotMsg = this.messagesOfOrigin.some(
        (m) => m.fromUID === channelInfo.channel.channelID
      );
      if (hasBotMsg) {
        this.rebuildRenderItems();
        this.notifyListener();
  ...
  async loadChannelInfoFinished() {
    ...
    if (this.channel.channelType === ChannelTypeCommunityTopic) {
      ...
      if (parentGroupNo) {
        ...
        const parentChannelInfo =
          WKSDK.shared().channelManager.getChannelInfo(parentChannel);
        const isSuperGroup =
          parentChannelInfo?.orgData?.group_type == SuperGroup;
        if (isSuperGroup) {
          // 超级群：只取第一页
          this.subscribers =
            await WKApp.dataSource.channelDataSource.subscribers(
              parentChannel,
              {
                limit: 100,
                page: 1,
          ...
          // 普通群：从缓存拿，没有则同步
          const cached =
            WKSDK.shared().channelManager.getSubscribes(parentChannel);
  ...
  appendMessage(messageWrap: MessageWrap) {
    ...
    const alreadyExists = msgKey
      ? this.messagesOfOrigin.some(
          (m) => (m.clientMsgNo || m.messageID?.toString()) === msgKey
        )
      : false;
    if (!alreadyExists) {
  ...
  deduplicateSystemTips(messages: Array<MessageWrap>): Array<MessageWrap> {
    ...
    return messages.filter((m) => {
      ...
      if (
        ...
        Math.abs(m.timestamp - lastTimestamp) < minIntervalSec
      ) {
  ...
  refreshMessages(
    messages: MessageWrap[],
    callback?: () => void,
    options?: { allowFoldAnimation?: boolean }
  ) {
